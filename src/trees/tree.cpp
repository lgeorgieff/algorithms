...
std::vector<tvalue> trees::tree<tkey, tvalue>::values(enum order order) const {
  ...
  switch (order) {
    case order::in_order:
      in_order_values<tkey, tvalue>(this->root_, result);
      break;
    case order::pre_order:
      pre_order_values<tkey, tvalue>(this->root_, result);
      break;
    case order::post_order:
      post_order_values<tkey, tvalue>(this->root_, result);
      break;
    default:
      throw std::invalid_argument("unknown order: " + std::to_string(order));
  }
...
std::vector<tkey> trees::tree<tkey, tvalue>::keys(enum order order) const {
  ...
  switch (order) {
    case order::in_order:
      in_order_keys<tkey, tvalue>(this->root_, result);
      break;
    case order::pre_order:
      pre_order_keys<tkey, tvalue>(this->root_, result);
      break;
    case order::post_order:
      post_order_keys<tkey, tvalue>(this->root_, result);
      break;
    default:
      throw std::invalid_argument("unknown order: " + std::to_string(order));
  }
